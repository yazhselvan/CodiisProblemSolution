...
public class AmazonEnd2EndTest extends testBase {
	...
	public void CompleteEnd2End(String LoginID, String Password, 
		...
		testUtil.extentTest.log(LogStatus.INFO, "Searched for " + SearchItem + " and Search Results are returned");
		...
		for (int i=0; i<searchResults.ProductList().size(); i++) {
			...
			testUtil.extentTest.log(LogStatus.INFO, "Selected the product " + searchResults.ProductList().get(i));
			searchResults.ValidateProduct(searchResults.ProductList().get(i));
			searchResults.AddItemstoCart();
			testUtil.extentTest.log(LogStatus.INFO, "Added " + searchResults.ProductList().get(i) + " to the cart");
		}
